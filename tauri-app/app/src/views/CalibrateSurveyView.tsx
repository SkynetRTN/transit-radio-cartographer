...
function bracketLayout(bracket: CalibrationBracket, fluxPad = 0.15, decPad = 0.15): BracketLayout {
  const fluxPoints: ScatterPoint[] = [];
  const decPoints: ScatterPoint[] = [];

  const pushPoints = (
    series: typeof bracket.on,
  ): { fluxes: number[]; decs: number[]; ras: number[] } => {
    const fluxes: number[] = [];
    const decs: number[] = [];
    const ras: number[] = [];
  ...
  // On/off separator: midpoint between the last on RA and the first off RA.
  ...
  const fluxYPad = Math.max((fmax - fmin) * fluxPad, 0.001);
  const dmin = Math.min(...decs);
  const dmax = Math.max(...decs);
  const decYPad = Math.max((dmax - dmin) * decPad, 0.01);
...
export function CalibrateSurveyView() {
  ...
  const loadView = useCallback(async () => {
    ...
    setError(null);
    try {
      const v = await rpcClient.getCalibrationView(workspaceHandle);
      setView(v);
    } catch (e) {
  ...
  const handleClick = useCallback((p: Point) => {
    // Click on a point always (re)pins it; clicking empty space unpins.
    setStickyPoint(p);
  }, []);
  ...
  // Which bracket panel currently owns the pin? Use RA range to decide so we
  // only draw the outline in the panel that actually contains the point.
  ...
  const makeDragHandlers = (bracket: 'initial' | 'terminal') => ({
    ...
    onDragEnd: async () => {
      ...
      if (mode !== 'cut' || !range || range.bracket !== bracket || workspaceHandle === null) {
        setDragRange(null);
        return;
      }
  ...
  const handleUndo = useCallback(async () => {
    if (workspaceHandle === null) return;
    try {
      await rpcClient.undoCalibrationCut(workspaceHandle);
      await Promise.all([loadView(), refreshWorkspace()]);
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  if (!workspace || workspaceHandle === null) {
    return (
      <div className="survey-view empty">
        <p>No survey workspace. Use Survey → New Survey... first.</p>
      </div>
    );
  }
  ...
  return (
    <div className="survey-view workspace calibrate-view">
      <div className="workspace-frame">
        <div className="workspace-title">{`${workspace.name} - Sweep 1`}</div>
